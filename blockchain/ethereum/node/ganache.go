...
func RunGanache(accountPrivateKeys ...string) (*EthClient, func(), error) {
	if IsProcessRunning(ganacheCliPort, "ganache-cli") {
	...
	cli, err := NewEthClient(fmt.Sprintf("http://127.0.0.1:%d", ganacheCliPort))
	if err != nil {
		killFunc()
		return nil, nil, err
	}
